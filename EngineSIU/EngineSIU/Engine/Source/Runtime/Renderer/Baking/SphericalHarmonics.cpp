...
namespace
{
    ...
    double Y(int32 l, int32 m, double Theta, double Phi)
    {
        if (m == 0)
        {
...
bool FSphericalHarmonics::SphericalHarmonicsFromHDRI(const FWString& FilePath, TArray<FVector>& OutResult)
{
    ...
    {
        float* RowPtr = reinterpret_cast<float*>(Image->pixels + Row * Image->rowPitch);
        for (size_t Col = 0; Col < ImageWidth; ++Col)
        {
            float* PixPtr = RowPtr + Col * 4;
    ...
    
    {
        ...
        for (uint64 Row = 0; Row < ImageHeight; ++Row)
        {
            const double Theta = (static_cast<double>(Row) + 0.5) / H * PI;
            const double D_Omega = FMath::Sin(Theta) * (PI / H) * (2.0 * PI / W);
            for (uint64 Column = 0; Column < ImageWidth; ++Column)
            {
                const double Phi = (static_cast<double>(Column) + 0.5) / W * 2.0 * PI;
                const FVector L = SampleColor(Image, Column, Row);
                ...
                for (int32 l = 0; l < NumBands; ++l)
                {
                    for (int32 m = -l; m <= l; ++m)
                    {
                        ...
                        OutResult[Idx] += L * static_cast<float>(Y(l, m, Theta, Phi) * D_Omega);
                    }
                }
            }
        }
