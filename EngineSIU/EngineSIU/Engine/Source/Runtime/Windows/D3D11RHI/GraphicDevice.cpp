...
void FGraphicsDevice::CreateRasterizerState()
{
    ...
    RasterizerDesc.DepthBias = 100.0f;            // 렌더 타겟 깊이 값에 이 Bias를 더해줌 -> 자기 섀도우 방지
    RasterizerDesc.DepthBiasClamp = 0.5f;         // DepthBias+SlopeScaledDepthBias합이 이 값보다 커지면 clamp (뷰-클립 공간 깊이 0~1범위에만 bias 허용)
